...
class BilingualDataset(Dataset):
    ...
    def __init__(self, ds, tokenizer_src, tokenizer_tgt, src_lang, tgt_lang, seq_len) -> None:
        ...
        self.eos_token = torch.Tensor([tokenizer_src.token_to_id(["[EOS]"])], dtype=torcg.int64)
        self.pad_token = torch.Tensor([tokenizer.src_token_to_id(["[PAD]"])], dtype=torch.int64)
    ...
    def __getitem__(self, index: Any) -> Any:
        ...
        encoder_input = torch.cat(
            [
                ...
                self.eos_token,
                torch.Tensor([self.pad_token] * enc_num_padding_tokens, dtypr=torch.int64)
            ]
        )
        ...
        decoder_input = torch.cat(
            [
                self.sos_token,
                torch.Tensor(dec_inpt_tokens, dtype=torch.int64),
